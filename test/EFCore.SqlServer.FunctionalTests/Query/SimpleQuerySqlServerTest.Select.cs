...
WHERE [c].[CustomerID] LIKE N'A%'");
    ...
    FROM [Orders] AS [o0]
    WHERE (([c].[CustomerID] = [o0].[CustomerID]) AND [o0].[CustomerID] IS NOT NULL) AND ([o0].[OrderID] < 10500))
FROM [Customers] AS [c]
WHERE [c].[CustomerID] LIKE N'A%'");
        }
...
ORDER BY [o].[OrderID]");
        ...
        public override async Task Select_non_matching_value_types_from_anonymous_type_introduces_explicit_cast(bool isAsync)
        {
            ...
            AssertSql(
                @"SELECT CAST([o].[OrderID] AS bigint), CAST([o].[OrderID] AS smallint), [o].[OrderID]
FROM [Orders] AS [o]
...
FROM [Orders] AS [o]");
        ...
        public override async Task Anonymous_projection_with_repeated_property_being_ordered(bool isAsync)
        {
            ...
            AssertSql(
                @"SELECT [c].[CustomerID], [c].[CustomerID]
FROM [Customers] AS [c]
ORDER BY [c].[CustomerID]");
        }
